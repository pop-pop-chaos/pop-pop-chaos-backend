...
try {
    ...
    $decrement = ($action === 'god_deflate') ? 10 : 1;
    $newSize = max(0, $currentSize - $decrement);

    if ($newSize <= 0) {
        // Bubble popped - delete it
        $stmt = $pdo->prepare("DELETE FROM bubbles WHERE bubble_id = ?");
        $stmt->execute([$bubbleId]);

        logBubbleEvent($bubbleId, 'popped', $currentSize, 0);
